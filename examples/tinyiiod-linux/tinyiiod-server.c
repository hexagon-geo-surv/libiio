		...
		const struct iio_context_params *params, const char *args)
{
	...
	if (iio_device_add_attr(dev, "name", IIO_ATTR_TYPE_DEVICE) < 0) {
		iio_context_destroy(ctx);
		return iio_err_cast(dev);
	}

	if (iio_device_add_attr(dev, "sampling_frequency", IIO_ATTR_TYPE_DEVICE) < 0) {
		iio_context_destroy(ctx);
		return iio_err_cast(dev);
	}
	...
	for (i = 0; i < ADC_NUM_CHANNELS; i++) {
		...
		if (iio_channel_add_attr(chn, "raw", IIO_ATTR_TYPE_CHANNEL, NULL) < 0 ||
		    iio_channel_add_attr(chn, "scale", IIO_ATTR_TYPE_CHANNEL, NULL) < 0 ||
		    iio_channel_add_attr(chn, "offset", IIO_ATTR_TYPE_CHANNEL, NULL) < 0) {
			iio_context_destroy(ctx);
			return iio_err_cast(chn);
		}
	}
	...
	for (i = 0; i < ADC_NUM_CHANNELS; i++) {
		...
		if (iio_buffer_add_scan_element(buf, chn, NULL) < 0) {
			iio_context_destroy(ctx);
			return iio_err_cast(dev);
		}
	}
