...
namespace iio
{
    ...
    public class BufferStream : IIOObject
    {
        ...
        [DllImport(IioLib.dllname, CallingConvention = CallingConvention.Cdecl)]
        private static extern int iio_buffer_stream_enable(IntPtr stream);

        [DllImport(IioLib.dllname, CallingConvention = CallingConvention.Cdecl)]
        private static extern int iio_buffer_stream_disable(IntPtr stream);
        ...
        public bool enabled {
            ...
            set {
                int err;
                if (value)
                    err = iio_buffer_stream_enable(hdl);
                else
                    err = iio_buffer_stream_disable(hdl);
                if (err != 0)
                    throw new IIOException("Unable to " + (value ? "en" : "dis") + "able buffer stream", err);
