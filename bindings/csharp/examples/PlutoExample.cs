...
namespace IIOCSharp
{
    static class PlutoExample
    {
        ...
        static void rx_config(Device device, double rx_lo, double rx_bw, double rx_fs, string gain_ctrl_mode)
        {
            ...
            device.get_channel("RX_LO", true).attrs.Find(a => a.name == "frequency").write(rx_lo);
            rx_chn.attrs.Find(a => a.name == "sampling_frequency").write(rx_fs);
            rx_chn.attrs.Find(a => a.name == "rf_bandwidth").write(rx_bw);
            rx_chn.attrs.Find(a => a.name == "gain_control_mode").write(gain_ctrl_mode);
        }
        ...
        static void tx_config(Device device, double tx_lo, double tx_bw, double tx_fs, double hw_gain)
        {
            ...
            device.get_channel("TX_LO", true).attrs.Find(a => a.name == "frequency").write(tx_lo);
            tx_chn.attrs.Find(a => a.name == "sampling_frequency").write(tx_fs);
            tx_chn.attrs.Find(a => a.name == "rf_bandwidth").write(tx_bw);
            tx_chn.attrs.Find(a => a.name == "hardwaregain").write(hw_gain);
        }
