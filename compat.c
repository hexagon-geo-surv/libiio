...
struct compat {
	...
	int (*iio_buffer_stream_enable)(struct iio_buffer_stream *);
	int (*iio_buffer_stream_disable)(struct iio_buffer_stream *);
					     ...
					     size_t samples_count, bool cyclic)
{
	...
	if (!dev_compat->is_tx) {
		...
		err = IIO_CALL(iio_buffer_stream_enable)(bs);
		if (err)
			goto err_free_blocks;
...
void iio_buffer_destroy(struct iio_buffer *buf)
{
	...
	if (compat->enabled)
		IIO_CALL(iio_buffer_stream_disable)(compat->bs);
...
ssize_t iio_buffer_push_partial(struct iio_buffer *buf, size_t samples_count)
{
	...
	if (!compat->enabled) {
		err = IIO_CALL(iio_buffer_stream_enable)(compat->bs);
		if (err)
			return err;
...
#define FIND_SYMBOL(lib, symbol) \
	...
	FIND_SYMBOL(ctx->lib, iio_buffer_stream_enable);
	FIND_SYMBOL(ctx->lib, iio_buffer_stream_disable);
